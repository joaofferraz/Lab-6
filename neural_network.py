...
class NeuralNetwork:
    ...
    def compute_gradient_back_propagation(self, inputs, expected_outputs):
        ...
        for i in range(num_cases):
            ...
            biases_gradient[2] += (outputs[i] - expected_outputs[i])
            weights_gradient[2] += (outputs[i] - expected_outputs[i]) * np.transpose(a[1])
            biases_gradient[1] += np.multiply(np.dot(np.transpose(self.weights[2]), outputs[i] - expected_outputs[i]), sigmoid_derivative(z[1]))
            weights_gradient[1] += np.multiply(np.dot(np.transpose(self.weights[2]), outputs[i] - expected_outputs[i]), sigmoid_derivative(z[1])) * np.transpose(a[0])
    ...
    def back_propagation(self, inputs, expected_outputs):
        ...
        for i in range(2):
            self.weights[i + 1] -= self.alpha * weights_gradient[i + 1]
            self.biases[i + 1] -= self.alpha * biases_gradient[i + 1]
